...
class DataPrepper:
    ...
    def __log_ltr_query_features(self, query_id, key, query_doc_ids, click_prior_query, no_results, terms_field="_id"):
        ...
        feature_log = {}
        feature_log["doc_id"] = []
        feature_log["query_id"] = []
        feature_log["sku"] = []
        for hit in response['hits']['hits']:
            feature_log["doc_id"].append(hit['_id'])
            feature_log["query_id"].append(query_id)
            feature_log["sku"].append(hit['_source']['sku'][0])
            features = hit['fields']['_ltrlog'][0]['log_entry']
            for feature in features:
                feature_name = feature['name']
                feature_value = feature.get('value', 0)
                if not feature_log.get(feature_name):
                    feature_log[feature_name] = []
                feature_log[feature_name].append(feature_value)
        frame = pd.DataFrame(feature_log)
